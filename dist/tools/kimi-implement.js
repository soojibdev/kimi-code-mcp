...
export const kimiImplementSchema = {
    task: z.string().describe('What to implement — be specific about target files, expected behavior, and acceptance criteria'),
    work_dir: z.string().describe('Absolute path to the codebase root directory'),
    allow_commit: z.boolean().default(false).describe('Allow Kimi to git commit after editing (default: false — file edits only, user reviews and commits)'),
    max_output_tokens: z.number().optional().describe('Max tokens in response (~4 chars/token). Default: 15000.'),
    resume_session: z.string().optional().describe('Resume a previous Kimi session by ID (from kimi_list_sessions)'),
};
...
function checkCleanGit(workDir) {
    try {
...
export async function kimiImplementHandler({ task, work_dir, allow_commit = false, max_output_tokens, resume_session, }) {
    if (!isKimiInstalled()) {
        return {
    ...
    if (gitError) {
        return {
            content: [{ type: 'text', text: `Error: ${gitError}` }],
            isError: true,
        };
    }
