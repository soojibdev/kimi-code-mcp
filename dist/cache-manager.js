...
export class CacheManager {
     ...
     */
    async getOrCreateSession(workDir, warmupOptions) {
     ...
     */
    async warmupCache(workDir, options) {
...
Acknowledge when you've loaded the main files (no detailed analysis needed).`;
        ...
        for (const line of lines) {
            try {
                ...
                if (parsed.metadata?.session_id)
                    return parsed.metadata.session_id;
            }
            catch { /* not JSON, skip */ }
        }
     ...
     */
    async computeCodebaseHash(workDir) {
     ...
     */
    async waitForWarmup(cacheKey, maxWaitMs = 60000) {
        const checkInterval = 100;
        const startTime = Date.now();
        while (Date.now() - startTime < maxWaitMs) {
            const entry = this.cache.get(cacheKey);
            if (!entry)
                return undefined; // Entry was removed
            if (!entry.isWarming)
                return entry.sessionId; // Warming complete
